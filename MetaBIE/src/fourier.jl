struct FourierSeries{N,T,P}
    coeffs::Array{T,N}
    period::NTuple{N,P}
end

FourierSeries(coeffs::Vector, period::Number) = FourierSeries(coeffs, tuple(period))
(f::FourierSeries{1})(x::Number) = fourier_kernel(f.coeffs, x, inv(f.period[1]))
